...
``tai42_config_k8s`` is on the allowlist. The rule (see the README): the shipped
package imports
...
substrate (tai42-contract + tai42-kit, pydantic-settings) and their dependency
closure ONLY,
...
The runtime walk imports ``tai42_config_k8s`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_config_k8s`` means the assertion covers the SHIPPED package's
...
# Every third-party root the shipped ``tai42_config_k8s`` graph pulls in -- the declared runtime dependencies plus their
