...
``tai_config_k8s`` is on the allowlist. The rule (see the README): the shipped
package imports
the YAML stack behind the config manager (PyYAML, ruamel.yaml, pyaml-env), the
``kubernetes`` client from the optional ``[k8s]`` extra, the shared platform
substrate (tai-contract + tai-kit, pydantic-settings) and their dependency
closure ONLY,
...
The runtime walk imports ``tai_config_k8s`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_config_k8s`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_config_k8s"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_config_k8s`` graph pulls in -- the declared runtime dependencies plus their
# resolved closure. Adding a runtime dependency that brings a new root means adding that root here, but only when it
# is a genuine dependency of the shipped package -- the walks below are never widened just to make the test pass.
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_config_k8s graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_config_k8s module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_config_k8s sources: {offenders}"
