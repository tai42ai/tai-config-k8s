...
def test_detect_namespace_propagates_permission_error(
    ...
    """A non-absent OSError (e.g. PermissionError) on the SA file must not be
    swallowed as 'default' — it propagates loudly."""
...
def test_explicit_namespace_skips_detection(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    """An explicitly-set namespace must win — the after-validator must not
    overwrite it with the detected service-account value."""
    ns_file = tmp_path / "namespace"
